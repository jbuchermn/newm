...
- [ ] Misplaced popups when output isn't placed at 0, 0
- [ ] Large scale (?)
*** STRT DBeaver crash seems to be able to create orphan states
*** STRT Every size negotiation with LibreOffice fails
*** STRT Background rounding - prevent single row or col of missed pixels

** TODO Reduce number of Init / Show calls (or at least log messages)
