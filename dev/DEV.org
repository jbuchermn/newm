...
*** DONE Bug: XWayland Firefox does not map

** DONE Overview per output (configurable)

** TODO Background: Configurable w0, h0 and per output background, as well as additional no_anim flag
** DONE Background: Set workspace to enable accurate rounding even when another output with scale == 1 is present
