...
*** WAIT Bug: Occasionally Chromium opens with twice or half the size (happens more when newm-sidecar is running)

*** STRT Bug: obs-studio issues
- [ ] Misplaced popups when output isn't placed at 0, 0 (happens on sway as well)
- [ ] Large scale (?)
*** STRT DBeaver crash seems to be able to create orphan states
*** TODO Every size negotiation with LibreOffice fails
